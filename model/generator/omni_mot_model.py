...
class OmniMoTModel(ImaginaireModel):
    ...
    def training_step(
        ...
        with profile_cpu(self, "sequence_plan"):
    ...
    def _prepare_inference_data(
        ...
        with profile_cuda(self, "get_data_and_condition"):
        ...
        with profile_cpu(self, "tokenize_text"):
        ...
        with profile_cuda(self, "initial_pack"):
            packed_sequence = self._pack_input_sequence(
                sequence_plans,
                cond_text_tokens,
                gen_data_clean,
    ...
    def _get_velocity(
        ...
        *,
        ...
        if packed_sequence_template is None:
            with profile_cuda(self, "pack_per_step"):
                packed_sequence = self._pack_input_sequence(
                    sequence_plans,
                    text_tokens,
        ...
        # --- Network forward ---
        with profile_cuda(self, "denoise_total"):
            out = self.denoise(
                net=net,
                data_batch_packed=packed_sequence,
                memory=memory,
            )
        profile_increment(self, "diffusion_network_calls")
    ...
    def _run_classifier_free_guidance(
        ...
        if self.parallel_dims is None or not self.parallel_dims.cfgp_enabled:
            with profile_cuda(self, "conditional_forward"):
                cond_v = single_velocity_fn(cond_tokens, False)
            profile_increment(self, "conditional_forward_calls")
            with profile_cuda(self, "unconditional_forward"):
                uncond_v = single_velocity_fn(uncond_tokens, skip_text_tokens_for_cfg)
            profile_increment(self, "unconditional_forward_calls")
    ...
    def generate_samples_from_batch(
        ...
        **kwargs,
        ...
        with profile_cuda(self, "reasoner_ar") if upsample_task is not None else nullcontext():
        ...
        with profile_cuda(self, "prepare_data_total"):
            (
                sequence_plans,
                gen_data_clean,
                cond_tokens,
        ...
        try:
            ...
            def velocity_fn(noise_x: list[torch.Tensor], timestep: torch.Tensor) -> list[torch.Tensor]:
                ...
                if not _any_needs_text_cfg and velocity_postprocess is None:
                    with profile_cuda(self, "conditional_forward"):
                        cond_v = _single_velocity_fn(cond_tokens, skip_text_tokens=False)
                    profile_increment(self, "conditional_forward_calls")
                    return cond_v
            ...
            if isinstance(sampler, FixedStepSampler) or scheduler_type == "unipc":
                with profile_cuda(self, "sampler_total"):
                    latents = sampler(
                        velocity_fn,
                        initial_noise,
                        num_steps=num_steps,
                ...
                if _extra_num_steps > 0:
                    ...
                    with profile_cuda(self, "sampler_alignment_padding"):
                        _ = sampler(
                            velocity_fn,
                            latents,
        ...
        *,
        ...
        if num_views == 1:
            with profile_cuda(self, "vae_encode"):
                ...
                _, _, pixel_t, pixel_h, pixel_w = state.shape
                profile_flops(
                    self,
                    "vae_encode",
                    int(compute_wan_vae_encoder_flops(B=1, T=pixel_t, H=pixel_h, W=pixel_w)),
                )
            profile_increment(self, "vision_encode_calls")
            with profile_cuda(self, "vae_output_cast"):
                return encoded.contiguous().float()  # [...,C_latent,T_latent,H_latent,W_latent]
        ...
        for view_idx in range(num_views):
            ...
            with profile_cuda(self, "vae_encode"):
                encoded_view = self.encode(view_state)
                _, _, pixel_t, pixel_h, pixel_w = view_state.shape
                profile_flops(
                    self,
                    "vae_encode",
                    int(compute_wan_vae_encoder_flops(B=1, T=pixel_t, H=pixel_h, W=pixel_w)),
            ...
            profile_increment(self, "vision_encode_calls")
            with profile_cuda(self, "vae_output_cast"):
                encoded_view = encoded_view.contiguous().float()
    ...
    def get_data_and_condition(
        ...
        with profile_cuda(self, "vae_encode_total"):
    ...
    def denoise(
        ...
        """
        net = net or self.net
        with profile_cuda(self, "network_forward"):
            out_net = net(

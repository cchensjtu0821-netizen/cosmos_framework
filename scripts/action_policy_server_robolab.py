...
from cosmos_framework.utils.module_profiler import ModuleProfiler
...
class RobolabServerArgs(pydantic.BaseModel):
    ...
    """Mark this many initial requests as warm-up in profiling output."""
    profile_output_dir: Path = Path("/tmp/cosmos3_action_profile")
    """Directory for module_profile.jsonl and optional PyTorch profiler traces."""
    torch_profiler_request: int = -1
    """Run the heavyweight PyTorch profiler for this one-based request id; -1 disables it."""
...
class RobolabPolicyService:
    ...
    def _append_profile_report(self, report: dict[str, Any]) -> None:
        self.args.profile_output_dir.mkdir(parents=True, exist_ok=True)
        output_path = self.args.profile_output_dir / "module_profile.jsonl"
    ...
    def _infer_profiled(self, obs: dict[str, Any], profiler: ModuleProfiler) -> dict[str, Any]:
        ...
        with self._lock:
            with torch.inference_mode(), profiler.cuda("generator_total"):
        ...
        with profiler.cpu("action_postprocess"):
    ...
    def infer(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        enabled = self.args.enable_module_profile and request_id % self.args.profile_every_n == 0
        profiler = ModuleProfiler(
            enabled=enabled,
            request_id=request_id,
            warmup=request_id <= self.args.profile_warmup_requests,
        )
        if enabled:
            torch.cuda.reset_peak_memory_stats()
        self.model._module_profiler = profiler if enabled else None
        if hasattr(self.model, "net"):
            self.model.net._module_profiler = profiler if enabled else None

        torch_prof = None
        observed_flops = None
        try:
            if request_id == self.args.torch_profiler_request:
                activities = [
                ...
                with torch.profiler.profile(
                    ...
                    with profiler.cpu("request_total"):
                        outputs = self._infer_profiled(obs, profiler)
            else:
                with profiler.cpu("request_total"):
                    outputs = self._infer_profiled(obs, profiler)
        finally:
            self.model._module_profiler = None
            if hasattr(self.model, "net"):
                self.model.net._module_profiler = None

        if torch_prof is not None:
            self.args.profile_output_dir.mkdir(parents=True, exist_ok=True)
            trace_path = self.args.profile_output_dir / f"request_{request_id}_trace.json"
            torch_prof.export_chrome_trace(str(trace_path))
            averages = torch_prof.key_averages()
            observed_flops = sum(event.flops or 0 for event in averages)
            summary_path = self.args.profile_output_dir / f"request_{request_id}_summary.txt"
        ...
        if enabled:
            ...
            log.info(
                f"[robolab-policy-server][profile] request={request_id} "
                f"warmup={report['warmup']} generator_ms={generator_ms} "
                f"peak_allocated_mb={report.get('memory', {}).get('peak_allocated_mb')}"
            )
        return outputs
